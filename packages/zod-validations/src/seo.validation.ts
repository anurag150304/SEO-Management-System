...
const focusKeywordsValidation = z.preprocess(
  (val) => {
...
const seoBaseFields = {
  metaTitle: z
    .preprocess(
      ...
      z.string().max(256, "Meta title must be no more than 256 characters.").optional().nullable()
    ),
  ...
  canonicalUrl: z.preprocess(
    emptyStringToUndefined,
    z.url({ error: "Canonical URL must be a valid URL." }).max(500).optional().nullable()
  ),
  ...
  ogTitle: z.preprocess(
    emptyStringToUndefined,
    z.string().max(256, "OG title must be no more than 256 characters.").optional().nullable()
  ),
  ...
  ogImage: z.preprocess(
    emptyStringToUndefined,
    z.url({ error: "OG image must be a valid URL." }).max(500).optional().nullable()
  ),

  twitterTitle: z.preprocess(
    emptyStringToUndefined,
    z.string().max(256, "Twitter title must be no more than 256 characters.").optional().nullable()
  ),
  ...
  twitterImage: z.preprocess(
    emptyStringToUndefined,
    z.url({ error: "Twitter image must be a valid URL." }).max(500).optional().nullable()
  ),
};

export const createSeoSchema = z.object(seoBaseFields).refine(
  (data) => hasAtLeastOneField(data, ["robotsIndex", "robotsFollow"]),
  {
...
export const updateSeoSchema = z.object({
  ...
  (data) => hasAtLeastOneField(data, ["seoId", "robotsIndex", "robotsFollow"]),
  {
