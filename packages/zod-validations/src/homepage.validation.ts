...
const homepageBaseFields = {
  heroHeading: z
    .preprocess(
      ...
      z.string().max(256, "Hero heading must be no more than 256 characters.").optional().nullable()
    ),

  heroSubheading: z
    .preprocess(
      emptyStringToUndefined,
      z.string().max(256, "Hero subheading must be no more than 256 characters.").optional().nullable()
    ),

  heroImage: z
    .preprocess(
      emptyStringToUndefined,
      z.url({ error: "Hero image must be a valid URL." }).max(500).optional().nullable()
    ),

  heroCtaText: z
    .preprocess(
      emptyStringToUndefined,
      z.string().max(100, "Hero CTA text must be no more than 100 characters.").optional().nullable()
    ),

  heroCtaUrl: z
    .preprocess(
      emptyStringToUndefined,
      z.string().max(500, "Hero CTA URL must be no more than 500 characters.").optional().nullable()
    ),

  aboutTitle: z
    .preprocess(
      emptyStringToUndefined,
      z.string().max(256, "About title must be no more than 256 characters.").optional().nullable()
    ),
  ...
  aboutImage: z
    .preprocess(
      emptyStringToUndefined,
      z.url({ error: "About image must be a valid URL." }).max(500).optional().nullable()
    ),
};

export const createHomepageSchema = z.object(homepageBaseFields).refine(
...
export const updateHomepageSchema = z.object({
  ...
  (data) => hasAtLeastOneField(data, ["id"]),
  {
